...
struct LogMessage_lyd* d_messages[MAX_GAUGE_BID];
...
int main(int argc, char* argv[])
{
  ...
  // Allocate memory for each d_messages_para element
  for (int i = 0; i < MAX_GAUGE_BID; i++) {
      CUDACHECK(cudaMalloc(&d_messages[i], sizeof(LogMessage_lyd)));
      CUDACHECK(cudaMemset(d_messages[i], 0, sizeof(LogMessage_lyd)));
  }
  ...
  // Create an array of host pointers to store the copied data from the device
  LogMessage_lyd* h_messages[MAX_GAUGE_BID];

  // Allocate memory for each LogMessage_lyd on the host
  for (int i = 0; i < MAX_GAUGE_BID; i++) {
      h_messages[i] = new LogMessage_lyd;
  }

  // Copy the device messages (d_messages[i]) back to the host (h_messages[i])
  for (int i = 0; i < MAX_GAUGE_BID; i++) {
      cudaMemcpy(h_messages[i], d_messages[i], sizeof(LogMessage_lyd), cudaMemcpyDeviceToHost);
  }
  ...
  if (myRank == 0) {
    ...
    for (size_t i = 0; i < N_CHUNKS; ++i) { 
      gauge_time = static_cast<double>(h_messages[0]->timeValue[1][i] - h_messages[0]->timeValue[0][i]) / GAUGE_GPU_FREQUENCY;
      ...
      gauge_time = static_cast<double>(h_messages[0]->timeValue[3][i] - h_messages[0]->timeValue[2][i]) / GAUGE_GPU_FREQUENCY;
  ...
  // Free the device memory of the gauge test
  for (int i = 0; i < MAX_GAUGE_BID; i++) {
      cudaFree(d_messages[i]);  // Free the device memory for each element in d_messages
  }

  // Free the host memory
  for (int i = 0; i < MAX_GAUGE_BID; i++) {
      delete h_messages[i];  // Delete the host memory for each element in h_messages
  }
