...
# $MPI_HOME/bin/mpirun -np 8 --hostfile $hostfile -ppn 2 \
# bash -c 'nsys profile --force-overwrite true -o msccl-output/nsys_test_msccl_profile_rank${OMPI_COMM_WORLD_RANK:-$PMI_RANK} --trace=cuda,nvtx,osrt --stats=true $NCCLTESTS_MSCCL_SRC_LOCATION/build/all_reduce_perf -b 16M -e 16M -f 2 -g 1 -n 60' \
# &>> output.log
