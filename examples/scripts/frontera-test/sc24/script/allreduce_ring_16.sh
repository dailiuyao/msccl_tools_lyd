...
#SBATCH -o ../log/paper0/ccl-run-16nodes-64gpus-ring.o%j       # Name of stdout output file
#SBATCH -e ../log/paper0/ccl-run-16nodes-64gpus-ring.e%j       # Name of stderr error file
...
##SBATCH -A ccl-run-16nodes-64gpus-ring        # Project/Allocation name (req'd if you have more than 1)
...
export NCCLTESTS_MSCCL_SRC_LOCATION="/home1/09168/ldai1/ccl-build/nccl-tests-profile-msccl"
    ...
    for nchannel in "${nchannel_values[@]}"; do
        for nchunks in "${nchunks_values[@]}"; do
            for trees in "${trees_values[@]}"; do
                ...
                > /home1/09168/ldai1/ccl-build/msccl_tools_lyd/examples/scripts/frontera-test/sc24/log/paper0/chunk_step_1/ring/all-reduce_sum_float_ring_node${nnodes}_gpu$((nnodes*ngpus))_mcl${nchannel}_mck${nchunks}_i0.out
            done
        done
    done
done
