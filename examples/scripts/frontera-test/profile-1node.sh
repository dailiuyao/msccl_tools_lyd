...
#SBATCH -J ccl-run-1node           # Job name
#SBATCH -o ./log/ccl-run-1node.o%j       # Name of stdout output file
#SBATCH -e ./log/ccl-run-1node.e%j       # Name of stderr error file
...
##SBATCH -A ccl-run-1node       # Project/Allocation name (req'd if you have more than 1)
...
# $MPI_HOME/bin/mpirun -np 16 -ppn 4 $NCCLTESTS_NCCL_PROFILE_SRC_LOCATION/build/all_reduce_perf -b 128 -e 512M -w 0 -f 2 -g 1 -n 10
...
export MSCCL_XML_FILES=/home1/09168/ldai1/ccl-build/msccl_tools_lyd/examples/xml/allreduce_binary_tree_p_gpu01_3nodes_channel2_chunk6.xml
export NCCL_ALGO=MSCCL,TREE,RING
export NCCL_DEBUG=TRACE
export NCCL_PROTO=Simple

$MPI_HOME/bin/mpirun -np 3 -ppn 3 $NCCLTESTS_MSCCL_SRC_LOCATION/build/all_reduce_perf -b 6M -e 6M -w 0 -f 2 -g 1 -n 1
