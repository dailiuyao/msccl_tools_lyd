    ...
    for nchannel in "${nchannel_values[@]}"; do
        for nchunks in "${nchunks_values[@]}"; do
            for trees in "${trees_values[@]}"; do
                ...
                > ${MSCCL_TOOLS_XML}/aws-test/32nic/allreduce_ring_node${nnodes}_gpu$((nnodes*ngpus))_mcl${nchannel}_mck$((nchunks))_gan0.xml
            done
        done
    done
done
    ...
    for nchannel in "${nchannel_values[@]}"; do
        for nchunks in "${nchunks_values[@]}"; do
            for trees in "${trees_values[@]}"; do
                ...
                --protocol=Simple --num_gpus=$ngpus --num_nodes=$nnodes --nchunks=$nchunks --nchannel=$nchannel --instances=1 --trees=$trees \
                > ${MSCCL_TOOLS_XML}/aws-test/32nic/allreduce_binary-tree_node${nnodes}_gpu$((nnodes*ngpus))_mcl${nchannel}_mck${nchunks}_gan0.xml
            done
        done
    done
done
...
# python3 /home/liuyao/scratch/deps/msccl_tools_lyd/examples/mscclang/basic_msccl/allreduce_binary_tree.py \
# --protocol=Simple --num_gpus=16 --instances=1 --trees=1 \
# > ${MSCCL_TOOLS_XML}/basic_msccl/allreduce_basic_binary_tree_16gpus_1tree.xml[]

# python3 /home/liuyao/scratch/deps/msccl_tools_lyd/examples/mscclang/basic_msccl/allreduce_binary_tree.py \
# --protocol=Simple --num_gpus=16 --instances=1 --trees=2 \
# > ${MSCCL_TOOLS_XML}/basic_msccl/allreduce_basic_binary_tree_16gpus_2tree.xml
...
# python3 /home/liuyao/scratch/deps/msccl_tools_lyd/examples/mscclang/basic_msccl/allreduce_ring.py \
# --num_gpus=16 --instances=1 \
# > ${MSCCL_TOOLS_XML}/basic_msccl/allreduce_basic_ring_16gpus.xml
