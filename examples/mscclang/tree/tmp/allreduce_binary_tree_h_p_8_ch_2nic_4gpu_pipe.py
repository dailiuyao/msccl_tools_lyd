...
def allreduce_binary_tree_hierarchical(num_nodes, num_local_gpus, num_chunks, num_channel, instances, protocol):
    ...
    with MSCCLProgram("allreduce_binary_tree_hierarchical", topology, collective, instances, protocol=protocol):
        ...
        while step <= 2**num_level:       
            for rank in range(step, num_nodes, step*2):
                ...
                for pipe_step in range(0, pipe_size):    
                    ...
                    chunk(rank*num_local_gpus+inter_gpu_offset, Buffer.input, 0*pipe_size+pipe_step).reduce(c1, ch=0*num_channel_intra_stage+2*num_channel_per_stage)
        ...
        for pipe_step in range(0, pipe_size):   
            c1 = chunk(peer_0*num_local_gpus+inter_gpu_offset, Buffer.input, 0*pipe_size+pipe_step)
            chunk(0*num_local_gpus+inter_gpu_offset, Buffer.input, 0*pipe_size+pipe_step).reduce(c1, ch=0*num_channel_intra_stage+2*num_channel_per_stage)         
                
         # Broadcast tree - root is Rank 0
        for pipe_step in range(0, pipe_size):  
            chunk(0*num_local_gpus+inter_gpu_offset, Buffer.input, 0*pipe_size+pipe_step).copy(peer_0*num_local_gpus+inter_gpu_offset, Buffer.input, 0*pipe_size+pipe_step, ch=1*num_channel_intra_stage+2*num_channel_per_stage) 
        ...
        while step >= 2:       
            for rank in range(step, num_nodes, step*2):
                ...
                peer_0 = rank - low_bit
                for pipe_step in range(0, pipe_size):   
                    chunk(rank*num_local_gpus+inter_gpu_offset, Buffer.input, 0*pipe_size+pipe_step).copy(peer_0*num_local_gpus+inter_gpu_offset, Buffer.input, 0*pipe_size+pipe_step, ch=1*num_channel_intra_stage+2*num_channel_per_stage) 
                ...
                while peer_1 >= num_nodes:
                    ...
                    low_bit //= 2
                for pipe_step in range(0, pipe_size):   
                    chunk(rank*num_local_gpus+inter_gpu_offset, Buffer.input, 0*pipe_size+pipe_step).copy(peer_1*num_local_gpus+inter_gpu_offset, Buffer.input, 0*pipe_size+pipe_step, ch=1*num_channel_intra_stage+2*num_channel_per_stage) 
            step //= 2
        ...
        if (trees == 2) and (num_nodes % 2 == 0):
            ...
            while step <= 2**num_level:       
                for rank in range(step, num_nodes, step*2):
                    ...
                    for pipe_step in range(0, pipe_size):    
                        ...
                        chunk((num_nodes-1-rank)*num_local_gpus+inter_gpu_offset, Buffer.input, 1*pipe_size+pipe_step).reduce(c1, ch=0*num_channel_intra_stage+3*num_channel_per_stage)
            ...
            for pipe_step in range(0, pipe_size):  
                c1 = chunk((num_nodes-1-peer_0)*num_local_gpus+inter_gpu_offset, Buffer.input, 1*pipe_size+pipe_step)
                chunk((num_nodes-1-0)*num_local_gpus+inter_gpu_offset, Buffer.input, 1*pipe_size+pipe_step).reduce(c1, ch=0*num_channel_intra_stage+3*num_channel_per_stage)         
                    
            # Broadcast tree - root is Rank N-1
            for pipe_step in range(0, pipe_size): 
                chunk((num_nodes-1-0)*num_local_gpus+inter_gpu_offset, Buffer.input, 1*pipe_size+pipe_step).copy((num_nodes-1-peer_0)*num_local_gpus+inter_gpu_offset, Buffer.input, 1*pipe_size+pipe_step, ch=1*num_channel_intra_stage+3*num_channel_per_stage) 
            ...
            while step >= 2:       
                for rank in range(step, num_nodes, step*2):
                    ...
                    peer_0 = rank - low_bit
                    for pipe_step in range(0, pipe_size):  
                        chunk((num_nodes-1-rank)*num_local_gpus+inter_gpu_offset, Buffer.input, 1*pipe_size+pipe_step).copy((num_nodes-1-peer_0)*num_local_gpus+inter_gpu_offset, Buffer.input, 1*pipe_size+pipe_step, ch=1*num_channel_intra_stage+3*num_channel_per_stage) 
                    ...
                    while peer_1 >= num_nodes:
                        ...
                        low_bit //= 2
                    for pipe_step in range(0, pipe_size): 
                        chunk((num_nodes-1-rank)*num_local_gpus+inter_gpu_offset, Buffer.input, 1*pipe_size+pipe_step).copy((num_nodes-1-peer_1)*num_local_gpus+inter_gpu_offset, Buffer.input, 1*pipe_size+pipe_step, ch=1*num_channel_intra_stage+3*num_channel_per_stage) 
                step //= 2
