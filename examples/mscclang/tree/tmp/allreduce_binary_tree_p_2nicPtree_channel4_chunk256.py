...
def allreduce_binary_tree_hierarchical(num_nodes, num_local_gpus, num_chunks, num_channel, instances, protocol):
    ...
    with MSCCLProgram("allreduce_binary_tree_hierarchical", topology, collective, instances, protocol=protocol):
        ...
        # tree0: channel0 1->0->2->3
        # tree0: channel2 3->2->0->1
        ...
        for chunk_step in range(0, num_chunks_per_channel):
            for node in range(0, num_nodes):
                rank_offset = node * num_local_gpus
                for index in range(0, num_local_gpus-1):
                    other = chunk((gpu_index0[index])+rank_offset, Buffer.input, chunk_step+0*num_chunks_per_channel)
                    c1 = chunk((gpu_index0[index+1])+rank_offset, Buffer.input, chunk_step+0*num_chunks_per_channel)
                    c1.reduce(other, ch=0)

                for index in range(0, num_local_gpus-1):
                    other = chunk((gpu_index1[index])+rank_offset, Buffer.input, chunk_step+1*num_chunks_per_channel)
                    c2 = chunk((gpu_index1[index+1])+rank_offset, Buffer.input, chunk_step+1*num_chunks_per_channel)
                    c2.reduce(other, ch=1)
            ...
            while step >= 2:       
                for rank in range(step, num_nodes, step*2):
                    ...
                    low_bit = bit // 2
                    peer_0 = rank - low_bit
                    chunk(rank*num_local_gpus + gpu_index0[num_local_gpus-1], Buffer.input, chunk_step+0*num_chunks_per_channel).copy(peer_0*num_local_gpus + gpu_index0[num_local_gpus-1], Buffer.input, chunk_step+0*num_chunks_per_channel, ch=0) 
                    chunk(rank*num_local_gpus + gpu_index1[num_local_gpus-1], Buffer.input, chunk_step+1*num_chunks_per_channel).copy(peer_0*num_local_gpus + gpu_index1[num_local_gpus-1], Buffer.input, chunk_step+1*num_chunks_per_channel, ch=1) 
            ...
            for node in range(0, num_nodes):
                rank_offset = node * num_local_gpus
                for index in range(0, num_local_gpus-1):
                    c = chunk((gpu_index0[num_local_gpus - 1 - index]) + rank_offset, Buffer.input, chunk_step+0*num_chunks_per_channel)
                    c.copy((gpu_index0[num_local_gpus - 2 - index]) + rank_offset, Buffer.input, chunk_step+0*num_chunks_per_channel, ch=0)
                for index in range(0, num_local_gpus-1):
                    c = chunk((gpu_index1[num_local_gpus - 1 - index]) + rank_offset, Buffer.input, chunk_step+1*num_chunks_per_channel)
                    c.copy((gpu_index1[num_local_gpus - 2 - index]) + rank_offset, Buffer.input, chunk_step+1*num_chunks_per_channel, ch=1)
        ...
        # tree1: channel1 1-0-2-3
        # tree1: channel3 3-2-0-1
        ...
        if (trees == 2) and (num_nodes % 2 == 0):
            for chunk_step in range(0, num_chunks_per_channel):
                for node in range(0, num_nodes):
                    rank_offset = node * num_local_gpus
                    for index in range(0, num_local_gpus-1):
                        other = chunk((gpu_index0[index])+rank_offset, Buffer.input, chunk_step+2*num_chunks_per_channel)
                        c1 = chunk((gpu_index0[index+1])+rank_offset, Buffer.input, chunk_step+2*num_chunks_per_channel)
                        c1.reduce(other, ch=2)

                    for index in range(0, num_local_gpus-1):
                        other = chunk((gpu_index1[index])+rank_offset, Buffer.input, chunk_step+3*num_chunks_per_channel)
                        c2 = chunk((gpu_index1[index+1])+rank_offset, Buffer.input, chunk_step+3*num_chunks_per_channel)
                        c2.reduce(other, ch=3)
                ...
                while step >= 2:       
                    for rank in range(step, num_nodes, step*2):
                        ...
                        chunk((num_nodes-1-rank)*num_local_gpus+gpu_index0[num_local_gpus-1], Buffer.input, chunk_step+2*num_chunks_per_channel).copy((num_nodes-1-peer_0)*num_local_gpus+gpu_index0[num_local_gpus-1], Buffer.input, chunk_step+2*num_chunks_per_channel, ch=2) 
                        chunk((num_nodes-1-rank)*num_local_gpus+gpu_index1[num_local_gpus-1], Buffer.input, chunk_step+3*num_chunks_per_channel).copy((num_nodes-1-peer_0)*num_local_gpus+gpu_index1[num_local_gpus-1], Buffer.input, chunk_step+3*num_chunks_per_channel, ch=3) 
                ...
                for node in range(0, num_nodes):
                    rank_offset = node * num_local_gpus
                    for index in range(0, num_local_gpus-1):
                        c = chunk((gpu_index0[num_local_gpus - 1 - index]) + rank_offset, Buffer.input, chunk_step+2*num_chunks_per_channel)
                        c.copy((gpu_index0[num_local_gpus - 2 - index]) + rank_offset, Buffer.input, chunk_step+2*num_chunks_per_channel, ch=2)
                    for index in range(0, num_local_gpus-1):
                        c = chunk((gpu_index1[num_local_gpus - 1 - index]) + rank_offset, Buffer.input, chunk_step+3*num_chunks_per_channel)
                        c.copy((gpu_index1[num_local_gpus - 2 - index]) + rank_offset, Buffer.input, chunk_step+3*num_chunks_per_channel, ch=3)
