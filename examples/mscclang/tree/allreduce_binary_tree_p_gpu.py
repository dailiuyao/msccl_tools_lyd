...
def allreduce_binary_tree_hierarchical(num_nodes:int, num_local_gpus:int, num_chunks:int, num_channel:int, instances:int, protocol:str, trees:int):
    ...
    with MSCCLProgram("allreduce_binary_tree_hierarchical", topology, collective, instances, protocol=protocol):
        ...
        combined_indices = list(range(size))
        ...
        for chunk_step in range(0, num_chunks_per_channel):
            ...
            while step <= 2**num_level:       
                for rank in range(step, num_nodes, step*2):
                    ...
                    for channel in range(num_channel_per_tree):
                        c1 = chunk((peer_0 + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel)
                        chunk((rank + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel).reduce(c1, ch=channel)
                    ...
                    if peer_1 > rank:
                        ...
                        for channel in range(num_channel_per_tree): 
                            c1 = chunk((peer_1 + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel)
                            chunk((rank + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel).reduce(c1, ch=channel)
                step *= 2
            ...
            for channel in range(num_channel_per_tree): 
                c1 = chunk((peer_0 + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel)
                chunk((0 + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel).reduce(c1, ch=channel)                 
            
            
            # Broadcast tree - root is Rank 0
            for channel in range(num_channel_per_tree): 
                chunk((0 + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel).copy((peer_0 + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel, ch=channel) 
            ...
            while step >= 2:       
                for rank in range(step, num_nodes, step*2):
                    ...
                    for channel in range(num_channel_per_tree): 
                        chunk((rank + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel).copy((peer_0 + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel, ch=channel) 
                    ...
                    if peer_1 > rank:
                        for channel in range(num_channel_per_tree): 
                            chunk((rank + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel).copy((peer_1 + combined_indices[channel])%size, Buffer.input, chunk_step+channel*num_chunks_per_channel, ch=channel)
            ...
            for chunk_step in range(0, num_chunks_per_channel):
                ...
                while step <= 2**num_level:       
                    for rank in range(step, num_nodes, step*2):
                        ...
                        # the children node peer_0 is ready, reduce to the last-1 gpu in the parent node rank.
                        for channel in range(num_channel_per_tree):  
                            c1 = chunk(((num_nodes-1-peer_0)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel)
                            chunk(((num_nodes-1-rank)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel).reduce(c1, ch=channel+num_channel_per_tree)
                        ...
                        if peer_1 > rank:
                            # the children node peer_1 is ready, reduce to the last gpu in the parent node rank.
                            for channel in range(num_channel_per_tree):  
                                c1 = chunk(((num_nodes-1-peer_1)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel)
                                chunk(((num_nodes-1-rank)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel).reduce(c1, ch=channel+num_channel_per_tree) 
                    step *= 2
                ...
                for channel in range(num_channel_per_tree):
                    c1 = chunk(((num_nodes-1-peer_0)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel)
                    chunk(((num_nodes-1-0)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel).reduce(c1, ch=channel+num_channel_per_tree)     
     

                
                # Broadcast tree - root is Rank N-1
                for channel in range(num_channel_per_tree):
                    chunk(((num_nodes-1-0)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel).copy(((num_nodes-1-peer_0)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel, ch=channel+num_channel_per_tree) 
                ...
                while step >= 2:       
                    for rank in range(step, num_nodes, step*2):
                        ...
                        for channel in range(num_channel_per_tree):
                            chunk(((num_nodes-1-rank)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel).copy(((num_nodes-1-peer_0)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel, ch=channel+num_channel_per_tree) 
                        ...
                        for channel in range(num_channel_per_tree):
                            chunk(((num_nodes-1-rank)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel).copy(((num_nodes-1-peer_1)+combined_indices[channel])%size, Buffer.input, chunk_step+(channel+num_channel_per_tree)*num_chunks_per_channel, ch=channel+num_channel_per_tree) 
        ...
        #                 for channel in range(num_channel_per_tree): 
        #                     gpu_reduce(node_offset=((peer_0+1)%num_nodes), num_local_gpus=num_local_gpus, chunk_step=chunk_step, gpu_index=combined_indices[channel], num_chunks_per_channel=num_chunks_per_channel, ch_idx=channel+num_channel_per_tree)
        ...
        #                     for channel in range(num_channel_per_tree): 
        #                         gpu_reduce(node_offset=((peer_1+1)%num_nodes), num_local_gpus=num_local_gpus, chunk_step=chunk_step, gpu_index=combined_indices[channel], num_chunks_per_channel=num_chunks_per_channel, ch_idx=channel+num_channel_per_tree)
        ...
        #         for channel in range(num_channel_per_tree):
        #             gpu_reduce(node_offset=((0+1)%num_nodes), num_local_gpus=num_local_gpus, chunk_step=chunk_step, gpu_index=combined_indices[channel], num_chunks_per_channel=num_chunks_per_channel, ch_idx=channel+num_channel_per_tree)


        #         # Broadcast tree - root is Rank N-1
        #         # conduct the intra broadcast in the top level node from the last gpu
        #         for channel in range(num_channel_per_tree):
        #             gpu_broadcast(node_offset=((0+1)%num_nodes), num_local_gpus=num_local_gpus, chunk_step=chunk_step, gpu_index=combined_indices[channel], num_chunks_per_channel=num_chunks_per_channel, ch_idx=channel+num_channel_per_tree) 
        ...
        #         for channel in range(num_channel_per_tree):
        #             gpu_broadcast(node_offset=((peer_0+1)%num_nodes), num_local_gpus=num_local_gpus, chunk_step=chunk_step, gpu_index=combined_indices[channel], num_chunks_per_channel=num_chunks_per_channel, ch_idx=channel+num_channel_per_tree) 
        ...
        #                     for channel in range(num_channel_per_tree):
        #                         gpu_broadcast(node_offset=((peer_1+1)%num_nodes), num_local_gpus=num_local_gpus, chunk_step=chunk_step, gpu_index=combined_indices[channel], num_chunks_per_channel=num_chunks_per_channel, ch_idx=channel+num_channel_per_tree) 
