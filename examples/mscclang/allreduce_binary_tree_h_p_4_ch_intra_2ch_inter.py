...
def ring_all_gather(size, rank_offset=0, local_chunk_size=1, pipe_size=4, ch_offset=4):
    for ch in range(0, size):
        index = ch  * local_chunk_size
        for step in range(0, size-1):
            for pipe_step in range(0, pipe_size):
...
def allreduce_binary_tree_hierarchical(num_nodes, num_local_gpus, instances, protocol):
    ...
    with MSCCLProgram("allreduce_binary_tree_hierarchical", topology, collective, instances, protocol=protocol):
        ...
        for n in range(num_nodes):
            ring_reduce_scatter(size=num_local_gpus, rank_offset=n * num_local_gpus, local_chunk_size=1, pipe_size=pipe_size, ch_offset=0)
            ...
            for rank in range(step, num_nodes, step*2):
                ...
                for n in range(0, cross_node_chunk_size):
                    for pipe_step in range(0, pipe_size):    
                        ...
                        chunk(rank*num_local_gpus+n, Buffer.input, (0+n)*pipe_size+pipe_step).reduce(c1, ch=2*pipe_size+(pipe_step//2))
        ...
        for n in range(0, cross_node_chunk_size): 
            for pipe_step in range(0, pipe_size):   
                c1 = chunk(peer_0*num_local_gpus+n, Buffer.input, (0+n)*pipe_size+pipe_step)
                chunk(0*num_local_gpus+n, Buffer.input, (0+n)*pipe_size+pipe_step).reduce(c1, ch=2*pipe_size+(pipe_step//2))         
                
         # Broadcast tree - root is Rank 0
        for n in range(0, cross_node_chunk_size): 
            for pipe_step in range(0, pipe_size):  
                chunk(0*num_local_gpus+n, Buffer.input, (0+n)*pipe_size+pipe_step).copy(peer_0*num_local_gpus+n, Buffer.input, (0+n)*pipe_size+pipe_step, ch=2+2*pipe_size+(pipe_step//2)) 
        ...
        while step >= 2:       
            for rank in range(step, num_nodes, step*2):
                ...
                for n in range(0, cross_node_chunk_size): 
                    for pipe_step in range(0, pipe_size):   
                        chunk(rank*num_local_gpus+n, Buffer.input, (0+n)*pipe_size+pipe_step).copy(peer_0*num_local_gpus+n, Buffer.input, (0+n)*pipe_size+pipe_step, ch=2+2*pipe_size+(pipe_step//2)) 
                ...
                for n in range(0, cross_node_chunk_size):
                    for pipe_step in range(0, pipe_size):   
                        chunk(rank*num_local_gpus+n, Buffer.input, (0+n)*pipe_size+pipe_step).copy(peer_1*num_local_gpus+n, Buffer.input, (0+n)*pipe_size+pipe_step, ch=2+2*pipe_size+(pipe_step//2)) 
            step //= 2
        ...
        if (trees == 4) and (num_nodes % 2 == 0):
                ...
                for rank in range(step, num_nodes, step*2):
                    ...
                    for n in range(cross_node_chunk_size, cross_node_chunk_size*2):
                        for pipe_step in range(0, pipe_size):    
                            ...
                            chunk((num_nodes-1-rank)*num_local_gpus+n, Buffer.input, n*pipe_size+pipe_step).reduce(c1, ch=4+2*pipe_size+(pipe_step//2))
            ...
            for n in range(cross_node_chunk_size, cross_node_chunk_size*2):  
                for pipe_step in range(0, pipe_size):  
                    c1 = chunk((num_nodes-1-peer_0)*num_local_gpus+n, Buffer.input, n*pipe_size+pipe_step)
                    chunk((num_nodes-1-0)*num_local_gpus+n, Buffer.input, n*pipe_size+pipe_step).reduce(c1, ch=4+2*pipe_size+(pipe_step//2))         
                    
            # Broadcast tree - root is Rank N-1
            for n in range(cross_node_chunk_size, cross_node_chunk_size*2): 
                for pipe_step in range(0, pipe_size): 
                    chunk((num_nodes-1-0)*num_local_gpus+n, Buffer.input, n*pipe_size+pipe_step).copy((num_nodes-1-peer_0)*num_local_gpus+n, Buffer.input, n*pipe_size+pipe_step, ch=6+2*pipe_size+(pipe_step//2)) 
            ...
            while step >= 2:       
                for rank in range(step, num_nodes, step*2):
                    ...
                    for n in range(cross_node_chunk_size, cross_node_chunk_size*2):
                        for pipe_step in range(0, pipe_size):  
                            chunk((num_nodes-1-rank)*num_local_gpus+n, Buffer.input, n*pipe_size+pipe_step).copy((num_nodes-1-peer_0)*num_local_gpus+n, Buffer.input, n*pipe_size+pipe_step, ch=6+2*pipe_size+(pipe_step//2)) 
                    ...
                    for n in range(cross_node_chunk_size, cross_node_chunk_size*2): 
                        for pipe_step in range(0, pipe_size): 
                            chunk((num_nodes-1-rank)*num_local_gpus+n, Buffer.input, n*pipe_size+pipe_step).copy((num_nodes-1-peer_1)*num_local_gpus+n, Buffer.input, n*pipe_size+pipe_step, ch=6+2*pipe_size+(pipe_step//2)) 
                step //= 2
