...
def allreduce_binary_tree_hierarchical(num_nodes, num_local_gpus, instances, protocol):
    ...
    with MSCCLProgram("allreduce_binary_tree_hierarchical", topology, collective, instances, protocol=protocol):
        ...
        while step <= 2**num_level:       
            for rank in range(step, num_nodes, step*2):
                ...
                for n in range(0, cross_node_chunk_size):    
                    ...
                    chunk(rank*num_local_gpus+n, Buffer.input, 0+n).reduce(c1, ch=n)
        ...
        for n in range(0, cross_node_chunk_size):  
            c1 = chunk(peer_0*num_local_gpus+n, Buffer.input, 0+n)
            chunk(0*num_local_gpus+n, Buffer.input, 0+n).reduce(c1, ch=n)         
                
         # Broadcast tree - root is Rank 0
        for n in range(0, cross_node_chunk_size):  
            chunk(0*num_local_gpus+n, Buffer.input, 0+n).copy(peer_0*num_local_gpus+n, Buffer.input, 0+n, ch=n) 
        ...
        while step >= 2:       
            for rank in range(step, num_nodes, step*2):
                ...
                peer_0 = rank - low_bit
                for n in range(0, cross_node_chunk_size):  
                    chunk(rank*num_local_gpus+n, Buffer.input, 0+n).copy(peer_0*num_local_gpus+n, Buffer.input, 0+n, ch=n) 
                ...
                while peer_1 >= num_nodes:
                    ...
                    low_bit //= 2
                for n in range(0, cross_node_chunk_size): 
                    chunk(rank*num_local_gpus+n, Buffer.input, 0+n).copy(peer_1*num_local_gpus+n, Buffer.input, 0+n, ch=n) 
            step //= 2
        ...
        if (trees == 4) and (num_nodes % 2 == 0):
            ...
            while step <= 2**num_level:       
                for rank in range(step, num_nodes, step*2):
                    ...
                    for n in range(cross_node_chunk_size, cross_node_chunk_size*2):   
                        ...
                        chunk((num_nodes-1-rank)*num_local_gpus+n, Buffer.input, n).reduce(c1, ch=n)
            ...
            for n in range(cross_node_chunk_size, cross_node_chunk_size*2):   
                c1 = chunk((num_nodes-1-peer_0)*num_local_gpus+n, Buffer.input, n)
                chunk((num_nodes-1-0)*num_local_gpus+n, Buffer.input, n).reduce(c1, ch=n)         
                    
            # Broadcast tree - root is Rank N-1
            for n in range(cross_node_chunk_size, cross_node_chunk_size*2): 
                chunk((num_nodes-1-0)*num_local_gpus+n, Buffer.input, n).copy((num_nodes-1-peer_0)*num_local_gpus+n, Buffer.input, n, ch=n) 
            ...
            while step >= 2:       
                for rank in range(step, num_nodes, step*2):
                    ...
                    peer_0 = rank - low_bit
                    for n in range(cross_node_chunk_size, cross_node_chunk_size*2): 
                        chunk((num_nodes-1-rank)*num_local_gpus+n, Buffer.input, n).copy((num_nodes-1-peer_0)*num_local_gpus+n, Buffer.input, n, ch=n) 
                    ...
                    while peer_1 >= num_nodes:
                        ...
                        low_bit //= 2
                    for n in range(cross_node_chunk_size, cross_node_chunk_size*2): 
                        chunk((num_nodes-1-rank)*num_local_gpus+n, Buffer.input, n).copy((num_nodes-1-peer_1)*num_local_gpus+n, Buffer.input, n, ch=n) 
                step //= 2
