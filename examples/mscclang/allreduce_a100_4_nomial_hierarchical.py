    ...
    with MSCCLProgram("allreduce_4_nomial_tree", topology, collective, instances, protocol=protocol, interleaved_replication=False):
        ...
        while current_level >= 1:
            for rank in range (0, num_nodes, int(step_parent_parent)):
                if rank + step_parent_child < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk(rank*num_local_gpus+n, Buffer.input, n).copy((rank + int(step_parent_child))*num_local_gpus+n, Buffer.input, n)
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk(rank*num_local_gpus+n, Buffer.input, n).copy((rank + int(step_parent_child)*2)*num_local_gpus+n, Buffer.input, n)
                if rank + step_parent_child*3 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk(rank*num_local_gpus+n, Buffer.input, n).copy((rank + int(step_parent_child)*3)*num_local_gpus+n, Buffer.input, n)
            step_parent_child /= 4
            step_parent_parent /= 4
            current_level -= 1
            ...
            for rank in range (0, num_nodes, step_parent_parent):
                if rank + step_parent_child < num_nodes:
                    for n in range(0, cross_node_chunk_size):                    
                        ...
                        chunk((rank+inter_offset)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).reduce(c1)
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):                    
        ...
        while current_level > 1:
            for rank in range (0, num_nodes, int(step_parent_parent)):
                if rank + step_parent_child < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child) + inter_offset)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*2 + inter_offset)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*3 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*3 + inter_offset)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
            step_parent_child /= 4
            step_parent_parent /= 4
            current_level -= 1
        ...
        if current_level == 1:
            for rank in range (0, num_nodes, int(step_parent_parent)):
                ...
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*3 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
            step_parent_child /= 4
            step_parent_parent /= 4
            current_level -= 1
            ...
            for rank in range (0, num_nodes, step_parent_parent):
                if rank + step_parent_child < num_nodes:
                    for n in range(0, cross_node_chunk_size):                    
                        ...
                        chunk((rank+inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).reduce(c1)
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):                    
        ...
        while current_level > 1:
            for rank in range (0, num_nodes, int(step_parent_parent)):
                if rank + step_parent_child < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank + inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child) + inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank + inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*2 + inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*3 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank + inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*3 + inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
            step_parent_child /= 4
            step_parent_parent /= 4
            current_level -= 1
        ...
        if current_level == 1:
            for rank in range (0, num_nodes, int(step_parent_parent)):
                if rank + step_parent_child < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child))*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*2 - inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*3 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
            step_parent_child /= 4
            step_parent_parent /= 4
            current_level -= 1
        ...
        if current_level == 1:
            for rank in range (0, num_nodes, step_parent_parent):
                if rank + step_parent_child < num_nodes:
                    for n in range(0, cross_node_chunk_size):                    
                        c1 = chunk((rank + step_parent_child)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                        chunk((rank+inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).reduce(c1)
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):                    
                        c1 = chunk((rank + step_parent_child*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                        chunk((rank+inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).reduce(c1)
        ...
        while current_level > 1:
            for rank in range (0, num_nodes, int(step_parent_parent)):
                if rank + step_parent_child < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child) + inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*2 + inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*3 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*3 + inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
            step_parent_child /= 4
            step_parent_parent /= 4
            current_level -= 1
        ...
        if current_level == 1:
            for rank in range (0, num_nodes, int(step_parent_parent)):
                if rank + step_parent_child < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child))*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*2 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*2)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
                if rank + step_parent_child*3 < num_nodes:
                    for n in range(0, cross_node_chunk_size):
                        chunk((rank+inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset).copy((rank + int(step_parent_child)*3 - inter_offset*3)*num_local_gpus+n+intra_offset, Buffer.input, n+intra_offset)
            step_parent_child /= 4
