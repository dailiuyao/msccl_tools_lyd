...
def allreduce_binary_tree(size, instances, trees, protocol):
    ...
    with MSCCLProgram("allreduce_binary_tree", topology, collective, instances, protocol=protocol):
        ...
        while step <= 2**num_level:       
            for rank in range(step, size, step*2):
                ...
                for pipe_step in range(0, pipe_size):
                    c1 = chunk(peer_0, Buffer.input, pipe_step)
                    chunk(rank, Buffer.input, pipe_step).reduce(c1, ch=pipe_step)
                ...
                while peer_1 >= size:
                    ...
                    low_bit //= 2
                for pipe_step in range(0, pipe_size):
                    c1 = chunk(peer_1, Buffer.input, pipe_step)
                    chunk(rank, Buffer.input, pipe_step).reduce(c1, ch=pipe_step)
            step *= 2
        ...
        for pipe_step in range(0, pipe_size): 
            c1 = chunk(peer_0, Buffer.input, pipe_step)
            chunk(0, Buffer.input, pipe_step).reduce(c1, ch=pipe_step)         
                
         # Broadcast tree - root is Rank 0
        for pipe_step in range(0, pipe_size): 
            chunk(0, Buffer.input, pipe_step).copy(peer_0, Buffer.input, pipe_step, ch=pipe_step) 
        ...
        while step >= 2:       
            for rank in range(step, size, step*2):
                ...
                peer_0 = rank - low_bit
                for pipe_step in range(0, pipe_size): 
                    chunk(rank, Buffer.input, pipe_step).copy(peer_0, Buffer.input, pipe_step, ch=pipe_step) 
                ...
                while peer_1 >= size:
                    ...
                    low_bit //= 2
                for pipe_step in range(0, pipe_size): 
                    chunk(rank, Buffer.input, pipe_step).copy(peer_1, Buffer.input, pipe_step, ch=pipe_step) 
            step //= 2
        ...
        if (trees == 2) and (size % 2 == 0):
            ...
            while step <= 2**num_level:       
                for rank in range(step, size, step*2):
                    ...
                    low_bit = bit // 2
                    peer_0 = rank - low_bit
                    for pipe_step in range(pipe_size, 2*pipe_size): 
                        c1 = chunk(size-1-peer_0, Buffer.input, pipe_step)
                        chunk(size-1-rank, Buffer.input, pipe_step).reduce(c1, ch=pipe_step)
                    ...
                    while peer_1 >= size:
                        ...
                        low_bit //= 2
                    for pipe_step in range(pipe_size, 2*pipe_size): 
                        c1 = chunk(size-1-peer_1, Buffer.input, pipe_step)
                        chunk(size-1-rank, Buffer.input, pipe_step).reduce(c1, ch=pipe_step)
                step *= 2
            ...
            for pipe_step in range(pipe_size, 2*pipe_size): 
                c1 = chunk(size-1-peer_0, Buffer.input, pipe_step)
                chunk(size-1-0, Buffer.input, pipe_step).reduce(c1, ch=pipe_step)         
                    
            # Broadcast tree - root is Rank N-1
            for pipe_step in range(pipe_size, 2*pipe_size): 
                chunk(size-1-0, Buffer.input, pipe_step).copy(size-1-peer_0, Buffer.input, pipe_step, ch=pipe_step) 
            ...
            while step >= 2:       
                for rank in range(step, size, step*2):
                    ...
                    peer_0 = rank - low_bit
                    for pipe_step in range(pipe_size, 2*pipe_size): 
                        chunk(size-1-rank, Buffer.input, pipe_step).copy(size-1-peer_0, Buffer.input, pipe_step, ch=pipe_step) 
                    ...
                    while peer_1 >= size:
                        ...
                        low_bit //= 2
                    for pipe_step in range(pipe_size, 2*pipe_size): 
                        chunk(size-1-rank, Buffer.input, pipe_step).copy(size-1-peer_1, Buffer.input, pipe_step, ch=pipe_step) 
                step //= 2
