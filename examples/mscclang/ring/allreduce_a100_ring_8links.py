    ...
    with MSCCLProgram(f"allreduce_ring_{channels}channelsperring", topology, collective, instances,
        ...
        for step in range(0, size-1):
            ...
            for index in range(0, size):
                rank = gpu_index[(index + step) % size]
                next_rank = gpu_index[(index + step + 1) % size]
                channel = (index%channels + 24)
                c = chunk(next_rank, Buffer.input, index + 24)
                c.reduce(chunk(rank, Buffer.input, index + 24), ch=channel, recvtb=channel, sendtb=channel)
        ...
        for step in range(0, size-1):
            ...
            for index in range(0, size):
                rank = gpu_index[(index + step) % size]
                next_rank = gpu_index[(index + step + 1) % size]
                channel = (index%channels + 28)
                c = chunk(next_rank, Buffer.input, index + 28)
                c.reduce(chunk(rank, Buffer.input, index + 28), ch=channel, recvtb=channel, sendtb=channel)
        ...
        for step in range(-1, size-2):
            ...
            for index in range(0, size):
                rank = gpu_index[(index + step) % size]
                c = chunk(rank, Buffer.input, index + 24)
                next_rank = gpu_index[(index + step + 1) % size]
                channel = (index%channels + 24)
                c = c.copy(next_rank, Buffer.input, index + 24, ch=channel, recvtb=channel, sendtb=channel)
                
        for step in range(-1, size-2):
            gpu_index = [2, 1, 0, 3]
            for index in range(0, size):
                rank = gpu_index[(index + step) % size]
                c = chunk(rank, Buffer.input, index + 28)
                next_rank = gpu_index[(index + step + 1) % size]
                channel = (index%channels + 28)
                c = c.copy(next_rank, Buffer.input, index + 28, ch=channel, recvtb=channel, sendtb=channel)
